...
sub main {
	...
	# 6. SAT solver の実行
	&execSATsolver($cnfFile);

	$t4 = Time::HiRes::time;
	$timeDetail{"solver"} = sprintf( "%.3f", $t4 - $t3 );

	# 7. 結果ファイルのデコード・検証
	&decodeRes( $firstNode, $R_Ref );
	$t5 = Time::HiRes::time;
